...
export default ({ children }: { children: ReactNode }) => {
  ...
  const fetchZonesProduction = useCallback(async () => {
    try {
      ...
      const data = await getAllZonesByActeur(acteurId);
  ...
  const createZone = useCallback(
    async (data: CreateZoneProductionData) => {
      try {
        setErrorZones(null);
        const acteurId = getActeurId();
        const zoneData = {
    ...
    [fetchZonesProduction, showNotification, getActeurId]
  );

  const updateZone = useCallback(
    async (id: string, data: Partial<CreateZoneProductionData>) => {
      try {
        setErrorZones(null);
  ...
  const fetchMagasins = useCallback(async () => {
    try {
      ...
      setErrorMagasins(null);
      const acteurId = getActeurId();
      const data = await getAllMagasinsByActeur(acteurId);
      console.log("listes des magasin pour user", data);
      setMagasins(data);
    } catch (error: any) {
  ...
  const createMagasinHandler = useCallback(
    async (data: CreateMagasinData) => {
      try {
        setErrorMagasins(null);
        const acteurId = getActeurId();
        const magasinData = {
          ...
          acteur: { idActeur: acteurId },
          niveau1Pays: {
            idNiveau1Pays: data.niveau1Pays.idNiveau1Pays,
          },
        };
    ...
    [fetchMagasins, showNotification, getActeurId]
  );

  const updateMagasinHandler = useCallback(
    async (id: string, data: Partial<CreateMagasinData>) => {
      try {
        setErrorMagasins(null);
        ...
        showNotification(
          "error",
          `Erreur mise à jour magasin: ${error.message}`
        );
        throw error;
      }
    },
  ...
  const deleteMagasinHandler = useCallback(
    async (id: string) => {
      try {
        setErrorMagasins(null);
        ...
        showNotification(
          "error",
          `Erreur suppression magasin: ${error.message}`
        );
        throw error;
      }
    },
  ...
  const createStockHandler = useCallback(
    ...
    [fetchStocks, showNotification, getActeurId]
  );

  const updateStockHandler = useCallback(
    async (id: string, data: Partial<CreateStockData>) => {
      try {
        setErrorStocks(null);
        ...
        showNotification(
          "error",
          `Erreur mise à jour produit: ${error.message}`
        );
        throw error;
      }
    },
  ...
  const deleteStockHandler = useCallback(
    async (id: string) => {
      try {
        setErrorStocks(null);
        ...
        showNotification(
          "error",
          `Erreur suppression produit: ${error.message}`
        );
        throw error;
      }
    },
  ...
  const fetchSpeculations = useCallback(async () => {
    try {
      ...
      setErrorSpeculations(null);
      const acteurId = getActeurId();
      const data = await getAllSpeculationsByActeur(acteurId);
