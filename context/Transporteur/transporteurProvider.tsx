...
import { CreateVehiculeData, TransporteurContextType, Vehicule } from "@/Types/transportType"
...
export default ({ children }: { children: ReactNode }) => {
    ...
    }, [clearErrors, fetchVehicules, fetchTypeVoitures, fetchMonnaies, fetchPays, showNotification])

    // Créer un véhicule
    const createVehicule = useCallback(async (data: CreateVehiculeData) => {
        try {
            setLoadingVehicules(true)
            setErrorVehicules(null)
            const vehiculeData = {
    ...
    const updateVehicule = useCallback(async (id: string, data: Partial<CreateVehiculeData>) => {
        try {
            setLoadingVehicules(true)
            setErrorVehicules(null)
            ...
            setErrorVehicules(error.message)
            showNotification('error', `Erreur mise à jour véhicule: ${error.message}`)
            throw error
        } finally {
            setLoadingVehicules(false)
        }
    ...
    const deleteVehicule = useCallback(async (id: string) => {
        try {
            setLoadingVehicules(true)
            setErrorVehicules(null)
            ...
            setErrorVehicules(error.message)
            showNotification('error', `Erreur suppression véhicule: ${error.message}`)
            throw error
        } finally {
            setLoadingVehicules(false)
        }
    ...
    return <TransporteurContext.Provider value={value}>{children}</TransporteurContext.Provider>
}